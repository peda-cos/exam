...
3. Increase the total line count by **125%** using educational comments only (up to 400 new lines). For files already processed with this prompt, update existing Notas instead of reapplying the 125% rule.
...
- When instructed with `Line Number Referencing = yes`, prefix each new comment with `Nota <number>` (e.g., `Nota 1`).
...
- If `Line Number Referencing = yes`, use Nota numbers to connect related explanations.
...
- **Line Number Referencing** (`yes/no`): Prepend comments with Nota numbers when `yes` (default `yes`).
